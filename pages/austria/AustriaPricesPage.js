...
export default function MongoDBAustriaPricesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/mongodb-austria', {
          cache: 'no-store',
        });
        if (!res.ok) {
        ...
        const austriaData = json.austria || [];
        if (!austriaData.length) {
          console.warn('No austria data found in response');
          setData([]);
          setLoading(false);
          return;
        }

        console.log('Austria Data:', austriaData);
        setData(austriaData);
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>🇦🇹 Österreich Strompreise (MongoDB) – {selectedDate || 'Kein Datum ausgewählt'}</h1>
