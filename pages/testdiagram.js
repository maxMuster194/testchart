...
export default function StromChart() {
  ...
  const createAreaDataset = (label, data1, data2, color) => {
    const filledData = data1.map((val, i) => {
      if ((val > data2[i]) || (val < data2[i])) {
        return Math.min(val, data2[i]);
      } else {
        return null;
      }
    });
    ...
    return {
      ...
      pointRadius: 0,
      fill: {
        target: '+1', // Füllt bis zur nächsten Dataset
      },
      tension: 0.3,
    };
  };
  ...
  const getDataset = () => {
    ...
    if (view === 'all' || view === 'h0') {
      ...
      // Fläche zwischen H0 und Strompreis (orange)
      datasets.push(createAreaDataset('Differenz (H0)', h0, strompreis, 'rgba(255,165,0,0.3)'));
    }
    ...
    if (view === 'all' || view === 'pv') {
      ...
      // Fläche zwischen H0 mit PV und Strompreis (orange)
      datasets.push(createAreaDataset('Differenz (PV)', h0pv, strompreis, 'rgba(255,165,0,0.3)'));
    }
