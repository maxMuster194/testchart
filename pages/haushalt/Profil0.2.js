...
export default function StrompreisChart() {
  ...
  const [householdType, setHouseholdType] = useState('none'); // Standardmäßig "Bitte auswählen"
  ...
  const datasets = householdType === 'none' ? [] : [
    ...Object.keys(profileFactors).flatMap((profile) => {
      const factor = profileFactors[profile];
        ...
        datasetsForProfile.push(
          {
            ...
            borderColor: '#1E88E5', // Blau
            backgroundColor: 'rgba(30, 136, 229, 0.1)',
            fill: false,
            tension: 0.4,
            pointRadius: 0,
            pointHoverRadius: 0,
            hidden: !activeProfiles[profile],
          },
          {
            ...
            pointRadius: 0,
            pointHoverRadius: 0,
            hidden: !activeProfiles[profile] || isNaN(parsedCustomPrice) || parsedCustomPrice < 0,
          }
        );
  ...
  return (
    <div style={styles.container}>
      ...
      <h1 style={styles.title}>🇩🇪 Profile 0.2</h1>
      ...
      {!loading && !error && (
        <div>
          ...
          <div style={styles.buttonContainer}>
            {[1, 2, 3, 4, 5].map((profile) => (
              <button
                ...
                style={{
                  ...styles.toggleButton,
                  backgroundColor: activeProfiles[profile] ? styles.toggleButtonActive.backgroundColor : styles.toggleButton.backgroundColor,
                }}
                className="toggle-button"
              >
                {activeProfiles[profile] ? `Profil ${profile} ausblenden` : `Profil ${profile} einblenden`}
              </button>
            ))}
          </div>

          <p style={styles.activeStatus}>
            {Object.keys(activeProfiles).length > 0 && Object.keys(activeProfiles).some((p) => activeProfiles[p])
              ? `Aktive Profile: ${Object.keys(activeProfiles)
                  .filter((p) => activeProfiles[p])
                  .map((p) => `Profil ${p} (Faktor ${profileFactors[p]})`)
                  .join(', ')}`
              : 'Keine Profile aktiv'}
          </p>
          ...
          {householdType !== 'none' && Object.keys(activeProfiles).length > 0 && Object.keys(activeProfiles).some((p) => activeProfiles[p]) && (
