...
import DatePicker from 'react-datepicker'; // Kalender-Komponente
import 'react-datepicker/dist/react-datepicker.css'; // Stil für den Kalender

// ChartJS-Komponenten registrieren
ChartJS.register(LineElement, PointElement, LinearScale, CategoryScale, Title, Tooltip, Legend);

export default function H0Display() {
  const [h0Data, setH0Data] = useState([]);
  const [selectedDate, setSelectedDate] = useState(null); // null für DatePicker
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const response = await fetch('/api/h0');
        if (!response.ok) {
  ...
  const selectedData = h0Data.find((item) => item.date === formatDateForComparison(selectedDate));
