...
function App() {
  ...
  // Funktion zur Berechnung des stündlichen Verbrauchs
  const berechneStundenVerbrauch = () => {
    const stunden = Array(24).fill(0); // Array für 24 Stunden initialisieren

    Object.keys(standardVerbraucher).forEach((verbraucher) => {
      const watt = verbraucherDaten[verbraucher].watt || 0;
      if (watt <= 0) return; // Überspringe Verbraucher ohne Wattleistung

      const isGrundlast = ['kühlschrank', 'gefrierschrank', 'aquarium'].includes(verbraucher);
      const einstellung = erweiterteEinstellungen[verbraucher];

      if (isGrundlast) {
        // Grundlast-Verbraucher laufen 24 Stunden
        for (let i = 0; i < 24; i++) {
          stunden[i] += watt / 1000; // In kW umrechnen
        }
      } else {
        // Dynamische Verbraucher basierend auf Start- und Endzeit
